(function ($) {
    $(document).ready(function () {
            ...
            $.get('https://api.github.com/repos/freesewing/'+repo+'/issues', function( issues ) {
                ...
                if(issues.length > 0) {
                    ...
                    $.each(issues, function( index, issue ) {
                        $('#'+repo+'-issues').append(renderIssue(issue));
                    });
        ...
        function renderIssue(issue) {
            console.log(issue);
            var html = '<div class="mb-4 drop-shadow paper" id="'+issue.number+'">';
            html += '<p class="mb-0 py-0"><small>Issue <a href="#'+issue.number+'">#'+issue.number+'</a>, ';
